...
def UpdateData(modul_cards, product_cards, img_cards, manu):
            ...
            for el in element:
                for p in product_cards:
                    if p['name'] == el.name:
                        ...
                        Stinka.objects.filter(id = el.id).update(price=p['price'], pars_category='dytyachi')
            ...
            for p in product_cards:
                if p['modul_id'] == m['id']:
                    ...
                    Stinka.objects.create(id=new_sid, seria_id=new_id, manufacturer_id=manu,\
                                          dytyachi=True, name=p['name'], pars_name=p['name'], categori = 'STINKA',\
    ...
    #Видаляємо товар
    for m in Stinka.objects.filter(manufacturer_id=manu).filter(dytyachi=True):
            ...
            for p in product_cards:
                if p['modul_id'] == m['id']:
                    if Stinka.objects.filter(pars_name=p['prom']).filter(seria_id=modul_id):
                        ...
                        print('old:', p['name'])
                        Stinka.objects.filter(pars_name=p['prom']).filter(seria_id=modul_id).update(price=p['price'], pars_category='dytyachi')
                    else:
                        print('none:', p['name'])
                        new_db.append(p['prom'])
                        Stinka.objects.create(seria_id=modul_id, manufacturer_id=manu,\
                                          dytyachi=True, name=p['name'], pars_name=p['prom'], categori = 'STINKA',\
                                            description=p['des'], price=p['price'])
        #Додаємо новий товар
        else:
            new_id = Seria.objects.all().order_by('-id')[0].id + 1
            ...
            for p in product_cards:
                if p['modul_id'] == m['id']:
                    if Stinka.objects.filter(pars_name=p['prom']).filter(seria_id=modul_id):
                        new_db.append(p['prom'])
                        print('old:', p['name'])
                        Stinka.objects.filter(pars_name=p['prom']).filter(seria_id=modul_id).update(pars_category='dytyachi')
                    else:
                        print('none:', p['name'])
                        new_db.append(p['prom'])
                        Stinka.objects.create(seria_id=modul_id, manufacturer_id=manu,\
            ...
            for p in product_cards:
                if p['modul_id'] == m['id']:
                    if Stinka.objects.filter(pars_name=p['prom']).filter(seria_id=modul_id):
                        new_db.append(p['prom'])
                        print('old:', p['name'])
                        Stinka.objects.filter(pars_name=p['prom']).filter(seria_id=modul_id).update(price=p['price'])
                    else:
                        print('none:', p['name'])
                        new_db.append(p['prom'])
                        Stinka.objects.create(seria_id=modul_id, manufacturer_id=manu,\
...
def get_dytyachi_products_comfortmebli():
    path ="/home/ay507291/notacomforta.pl.ua/www"+File.objects.get(id=23).file_up.url
    print(path)
...
def get_dytyachi_products_matrolux():
    source = requests.get('https://matroluxe.ua/ua/mebel-dlya-detskoy', headers=HEADERS).text
    soup = BeautifulSoup(source, 'html.parser')
    time.sleep(1)
    
    pag_list = ['https://matroluxe.ua/ua/mebel-dlya-detskoy',]
    prod_link = []
...
def get_dytyachi_products_lion():
    modul_cards = []
    product_cards = []
    img_cards = []
    modul_id = 0
    prod_id = 0
    link_list = ['https://lion-mebli.com.ua/dytyachi/room-modules/',]


    source = requests.get('https://lion-mebli.com.ua/dytyachi/room-modules/',headers=HEADERS).text
    soup = BeautifulSoup(source, 'html.parser')
